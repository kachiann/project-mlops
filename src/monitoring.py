...
def analyze_data_quality(data, name):
    ...
    expected_zero_columns = ['holiday', 'workingday', 'casual', 'registered', 'zero_casual', 'zero_registered']
    
    # Check for zero values in numeric columns where zeros are unexpected
    numeric_columns = data.select_dtypes(include=[np.number]).columns
    unexpected_zero_columns = [col for col in numeric_columns if col not in expected_zero_columns]
    zeros = (data[unexpected_zero_columns] == 0).sum()
    if zeros.any():
        logger.warning(f"Unexpected zero values in numeric columns:\n{zeros[zeros > 0]}")
...
def compare_distributions(reference_data, production_data):
    ...
    for metric in key_metrics:
        ...
        logger.info(f"{metric} - Ref mean: {ref_mean:.2f}, Prod mean: {prod_mean:.2f}, Change: {change:.2f}%")
...
def generate_data_drift_report(reference_data, production_data, output_path):
    ...
    try:
        ...
        logger.error(f"Reference data shape: {reference_data.shape}, Production data shape: {production_data.shape}")
        raise
